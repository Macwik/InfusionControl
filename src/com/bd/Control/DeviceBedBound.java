...
public class DeviceBedBound {
	...
	public static boolean setDeviceStatusColor(String deviceID, EDeviceStatusColorEnum statusColor) {
		...
		if (di != null && (di.getStatusColor() == null || !di.getStatusColor().equals(statusColor))) {
			...
			// TODO 20180305 张超加
			// 设置面板颜色时，确定是否有报警
			setWarming(deviceID, statusColor);
			notifyUpdate(bedDeviceMap.get(deviceID));
			return true;
		}
		return false;
	...
	// TODO 20180305 张超加
	// 设置面板颜色时，确定是否有报警
	public static void setWarming(String deviceID, EDeviceStatusColorEnum statusColor) {
		if (statusColor.equals(EDeviceStatusColorEnum.EARLYWARNING)
				|| statusColor.equals(EDeviceStatusColorEnum.WARNING)) {
			BedWarmingMp3Manager.addWarming(deviceID);
		} else {
			BedWarmingMp3Manager.removeWarming(deviceID);
			/**
			 * TODO 2018年4月18日加
			 */
			// BedWarmingMp3Manager.removeQuit(deviceID);
		}
	}
